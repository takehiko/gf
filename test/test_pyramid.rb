...
=end
    ...
    4.upto(11) do |level|
      gf = GFLoad::Estimator.new(:level => level, :plc => 0)
      gf.start(true)
    ...
    4.upto(11) do |level|
      gf = GFLoad::Estimator.new(:level => level, :plc => 4)
      gf.start(true)
      assert_equal(person_a[level], gf.summary[:person])
